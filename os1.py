...
def create_canvas1(frame2):
    ...
    os1 = tk.Canvas(frame2, bg="peachpuff", width=690, height=460)
    os1.grid(row=0, column=0, sticky="nsew")

    os1_upper = tk.Frame(os1, width=690, height=80, bg='rosybrown')
    os1.create_window(0, 0, anchor='center', window=os1_upper)
    os1_upper.grid(sticky='nsew')
    ...
    # Create the buttons
    button1 = tk.Button(os1_upper, text="Announcement",command=show_announcement)
    button2 = tk.Button(os1_upper, text="Lectures",command=show_lecture)
    ...
    canvas_announcement = tk.Canvas(os1, bg="peachpuff", width=690, height=460)
    canvas_announcement.grid(row=1,column=0,sticky='nsew')

    canvas_lecture = tk.Canvas(os1, bg="lightpink", width=690, height=460)
    canvas_lecture.grid(row=1,column=0,sticky='nsew')
    

    
    file_list_frame = tk.Frame(canvas_announcement,width=690)
...
def refresh_file_list():
    ...
    for i, (data, timestamp, data_type) in enumerate(combined_data):
        ...
        if data_type == 0:  # File name
            ...
            label = tk.Label(frame, text=label_text, cursor="hand2", font=("Arial", 12))
            label.grid(row=0, column=0, sticky="w", padx=5, pady=2)
            download_button = tk.Button(frame, text=button_text, command=command)
            download_button.grid(row=0, column=1, padx=5, pady=2)
        else:  # Announcement message
            label_text = f"Announcement: {data} ({timestamp})"
            # Create label for announcement
            label = tk.Label(frame, text=label_text, cursor="hand2", font=("Arial", 12))
            label.grid(row=0, column=0, columnspan=2, sticky="w", padx=5, pady=2)
