...
from PIL import Image, ImageTk
from cn1 import create_canvas2
from os1 import create_canvas1
from coa1 import create_canvas3
import mysql.connector
from login_page_s import global_username
...
def open_excel_file():
    ...
    file_path = r"C:\Users\himes\OneDrive\Desktop\Mini Project Sem IV\ttd10a.xlsx"
    if file_path:
...
def login_page_s():
    ...
    login_page_s_instance = login_page_s.NewPage(root)
    login_page_s_instance.pack(fill=tk.BOTH, expand=True)
...
def toggle_frame_visibility():
    ...
    if frame1_visible:
        ...
        os1.grid_columnconfigure(0,weight=1)
        cn1.grid_columnconfigure(0,weight=1)
        coa1.grid_columnconfigure(0,weight=1)
        ...
        os1.grid_columnconfigure(0,weight=0)
        cn1.grid_columnconfigure(0,weight=0)
        coa1.grid_columnconfigure(0,weight=0)
...
def get_account_details(username):
    try:
        ...
        query = "SELECT name, email FROM student_details WHERE username = %s"
...
def toggle_dropdown():
    if dropdown_frame.winfo_ismapped():
        canvas2.itemconfigure(dropdown_button, text="Classrooms")
        dropdown_frame.place_forget()  # Use place_forget to hide the dropdown frame
    else:
        canvas2.itemconfigure(dropdown_button, text="Subjects")
...
frame3 = tk.Frame(root, width=935, height=90, bg="wheat4")
...
dropdown_button = canvas2.create_text(150, 20, text="Subjects", font=("Arial", 10), fill="pink", anchor="center")
...
tk.Label(dropdown_frame, text="Classrooms",width=28,height=3).pack()
heading_line = tk.Frame(dropdown_frame, height=2, bg="black")
heading_line.pack(fill=tk.X)

coa1=create_canvas3(frame2)
cn1=create_canvas2(frame2)
os1=create_canvas1(frame2)
...
tk.Button(dropdown_frame, text="OS",width=25,height=3, command=lambda: switch_canvas(os1)).pack()
tk.Button(dropdown_frame, text="CN",width=25,height=3, command=lambda: switch_canvas(cn1)).pack()
tk.Button(dropdown_frame, text="COA",width=25,height=3, command=lambda: switch_canvas(coa1)).pack()
...
def switch_canvas(canvas_to_show):
    ...
    button_data = [("MRS.RAKHI JADHAV -> OS", "hotpink", lambda: switch_canvas(os1)),
                ("MRS.ANUSHREE PRABHU -> CN", "lightsalmon", lambda: switch_canvas(cn1)),
                ("DR.ABHAY KSHIRSAGAR -> COA", "aquamarine", lambda: switch_canvas(coa1))]
...
show_details_button = tk.Button(frame3, text="Show Account Details", command=lambda: show_account_details())
