...
def create_canvas3(frame2):
    
    global canvas_announcement3, canvas_lecture3, default_message, file_list_frame3, chat_input3  # Use global keyword to modify global variables

    
    canvasd10c = tk.Canvas(frame2, bg="thistle", width=690, height=460)
    canvasd10c.grid(row=0, column=0, sticky="nsew")

    framed10c_upper = tk.Frame(canvasd10c, width=690, height=80, bg='lightsalmon')
    canvasd10c.create_window(0, 0, anchor='center', window=framed10c_upper)
    framed10c_upper.grid(sticky='nsew')
    ...
    # Create the buttons
    button1 = tk.Button(framed10c_upper, text="Announcement",command=show_announcement)
    button2 = tk.Button(framed10c_upper, text="Lectures",command=show_lecture)
    ...
    # Create canvas_announcement3, canvas_lecture3, file_list_frame3, and upload_button inside canvasd10b
    canvas_announcement3 = tk.Canvas(canvasd10c, bg="grey", width=690, height=460)
    canvas_announcement3.grid(row=1,column=0,sticky='nsew')

    canvas_lecture3 = tk.Canvas(canvasd10c, bg="peachpuff", width=690, height=460)
    canvas_lecture3.grid(row=1,column=0,sticky='nsew')
    
        # Create the text box inside the chatbox frame
    chat_input3 = tk.Text(canvas_announcement3, bg="white", width=60, height=4)  # Adjust width and height as needed
    chat_input3.grid(row=0, column=0, padx=5, pady=5, sticky="nsew")  # Adjust padding as needed

  
    # Set default message in the text box
    default_message = "Announce something to the class"
    chat_input3.insert(tk.END, default_message)
    chat_input3.tag_add("default", "1.0", "end")
    chat_input3.tag_config("default", foreground="darkgrey")  # Change the color of the default message
    chat_input3.bind("<Button-1>", clear_default_message)
        # Bind mousewheel event to enable scrolling
    chat_input3.bind("<MouseWheel>", on_mousewheel)
   
    # Create the "Post" button
    post_button = tk.Button(canvas_announcement3, text="Post", command=post_message,background="black" ,foreground="white",font="bold")
    post_button.grid(row=1, column=0, padx=10, pady=5,sticky="sw")  # Adjust padding as needed

    # Create the "Cancel" button
    cancel_button = tk.Button(canvas_announcement3, text="Cancel", command=cancel_message,background="black" ,foreground="white",font="bold")
    ...
    canvas_announcement3.grid_rowconfigure(0, weight=1)  
    
    file_list_frame3 = tk.Frame(canvas_announcement3)
    file_list_frame3.grid(row=2, column=0)

    refresh_file_list()
    
    upload_button = tk.Button(canvas_announcement3, text="Upload", command=upload_file,background="black" ,foreground="white",font="bold")
    upload_button.grid(row=1, column=2, padx=2, pady=2)
    
    fetch_and_display_videos(cursor, canvas_lecture3)
    
    return canvasd10c

def show_announcement():
    # Functionality for showing announcements
    canvas_announcement3.grid()
    canvas_lecture3.grid_forget()  # Hide the lecture chatbox canvas if it's visible

def show_lecture():
    canvas_lecture3.grid()
    canvas_announcement3.grid_forget()  # Hide the announcement chatbox canvas if it's visible  
...
def fetch_and_display_videos(cursor, canvas_lecture3):
    ...
    try:
        ...
        for widget in canvas_lecture3.winfo_children():
            widget.destroy()
        ...
        for video_name, video_data, timestamp in videos:
            ...
            video_button = Button(canvas_lecture3, text=video_button_text, command=lambda vd=video_data: play_video(vd),font=("Arial", 12))
...
def refresh_file_list():
    ...
    for widget in file_list_frame3.winfo_children():
        widget.destroy()
    ...
    for i, (data, timestamp, data_type) in enumerate(combined_data):
        if data_type == 0:  # File name
            ...
            label = tk.Label(file_list_frame3, text=label_text, cursor="hand2", bg="lightgray",font=("Arial", 12))
            label.grid(row=i, column=0, sticky="ew")
            download_button = tk.Button(file_list_frame3, text=button_text, command=command)
...
def post_message():
    ...
    message = chat_input3.get("1.0", tk.END).strip()
    ...
    if message and message != default_message:
        try:
            ...
            chat_input3.delete("1.0", tk.END)
            chat_input3.insert(tk.END, default_message)
            chat_input3.tag_add("default", "1.0", "end")
            chat_input3.tag_config("default", foreground="darkgrey")
...
def clear_default_message(event):
    ...
    if chat_input3.get("1.0", "end-1c") == "Announce something to the class":
        chat_input3.delete("1.0", tk.END) 